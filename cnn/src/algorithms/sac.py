...
class SAC(object):
	def __init__(self, obs_shape, action_shape, args):
		...
		if self.use_aux:
			print("shape of feature_map_head_true:",self.critic.encoder.head_cnn.out_shape_feature_map)
			print("shape of feature_map_shared_cnn_for_aux:",self.critic.encoder.shared_cnn.out_shape_for_aux)
			if args.head_true:
				self.decoder_head=m.PatchEmbed(embed_dim=128,norm_layer=None).cuda()
			else:
				self.decoder_head=m.PatchEmbed(embed_dim=128,norm_layer=None).cuda()
			self.aux_encoder = m.Encoder_for_aux(shared=shared_cnn,head=head_cnn,head_true=args.head_true,shared_cnn_layers=args.shared_cnn_layers).cuda()
